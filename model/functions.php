...
GROUP BY pro.clie_codigo, clie.clie_nombre ORDER BY pro.clie_codigo";
    $result = odbc_exec($conn, $query);
    while ($row = odbc_fetch_array($result)) {
      $row = array_map("utf8_encode", $row);
      if ($row['clie_codigo'] == 15) {
...
and op.orpr_numero = mov.defg_docrel where op.clie_codigo = ? and op.orpr_numero = ?";
  ...
  function getProductorProcesoReordena($conex, $cliente, $proceso)
  {
    $query = "SELECT prod_codigo FROM DBA.spro_ordenproceso where orpr_numero = ? and clie_codigo = ?";
    $resultQuery = odbc_prepare($conex, $query);
    odbc_execute($resultQuery, [$proceso, $cliente]);
    if (odbc_num_rows($resultQuery) == 0) {
      $detalle = ['error' => true];
  ...
  {
    $query = "SELECT (fg_deta.mfgd_kgnent * fg_deta.mfgd_bulent) as mfgd_kgnent, fg_enca.prod_codigo, fg_deta.lote_codigo, fg_deta.mfgd_bulent, fg_deta.mfgd_secuen FROM 
...
fg_deta.lote_espcod = ? and fg_deta.mfge_numero = ? order by fg_deta.mfgd_secuen";
    $resultQuery = odbc_prepare($conex, $query);
  ...
  function getLotesProceso($conex, $cliente, $orden)
  {
    $query = "SELECT orden.lote_codigo, orden.orpd_secuen, orden.orpd_canbul, (orden.orpd_canbul * deta.lotd_kilpro) as kilos FROM DBA.spro_ordenprocdeta as orden join
DBA.spro_lotesfrutagrandeta as deta on orden.lote_codigo = deta.lote_codigo and orden.lote_espcod = deta.lote_espcod
